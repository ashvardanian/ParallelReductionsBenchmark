...
void gForEachOpenCLMethod(sBenchDescription xBench,
                          sStr xProgramPath,
                          SArr<sStr> xMethods,
...
void nCL::gBenchReduce(sBenchDescription xBench,
                       SArr<sReal> const & xArrFullCPU,
                       SArr<sBenchResult> & yResults) {
    SArr<sReal> lArrPartialSumsCPU(xBench.mCountThreads);
    
    gForEachOpenCLMethod(xBench, kFileReduce, {
        "gReduceSimple", "gReduceWModulo",
        "gReduceInSharedMem", "gReduceWSequentialAddressing", "gReduce2Steps",
        "gReduceUnrolled", "gReduceFullyUnrolled", "gReduceWBrentsTh"
        ...
        lAppPtr->fReallocVar(1)->fResetVariable(lArrPartialSumsCPU.data(), sizeof(sReal) * lArrPartialSumsCPU.size());
...
void nCL::gBenchParallel(sBenchDescription xBench,
                         SArr<sReal> const & xArrFullCPU,
                         SArr<sBenchResult> & yResults) {
    ...
    auto fRunWithConfigurator = [&](SArr<sStr> xMethods, auto xConfigurator, bInt32 xOutputIndex) {
        gForEachOpenCLMethod(xBench, kFileParallel, xMethods, yResults, [&](SShared<sTask> lAppPtr, sStr lMethod) {
            gResetOnCPU(lArrResultCPU);
            lAppPtr->fReallocVar(0)->fResetConstant(xArrFullCPU.data(), sizeof(sReal) * xBench.mSize);
    ...
    fRunWithConfigurator({
        "gArithmAddConst", "gArithmMulConst",
    ...
    fRunWithConfigurator({
        "gPowSqrt", "gPowExp",
        "gTrigCos", "gTrigCosNative", // "gTrigCos16",
    ...
    fRunWithConfigurator({
        "gPowPositiveBase", "gPowIntExp", "gPowInverseIntExp",
    }, [&](SShared<sTask> lAppPtr, sStr lMethod) {
    ...
    fRunWithConfigurator({
        "gMulAddConst", "gMulAddConstManual",
    }, [&](SShared<sTask> lAppPtr, sStr lMethod) {
        cl_float lX = static_cast<cl_float>(M_PI);
        lAppPtr->fReallocVar(1)->fResetValue(&lX, sizeof(cl_float));
        lAppPtr->fReallocVar(2)->fResetValue(&lX, sizeof(cl_float));
